...
def parse_args():
    ...
    parser.add_argument('--path', type=str, default='input.png')
    parser.add_argument('--img_path', type=str, default='input.png')
    parser.add_argument('--datasetname', type=str, default='input.png')
...
def main():
    ...
    for folder in folders:
        for file in os.listdir(os.path.join(args.path, folder)):
            ...
            args.output_folder = os.path.join(output_folder1, folder, file_name)
            img_path = os.path.join(args.img_path, parent, file_name)
            if not os.path.exists(img_path):
                print(f'Path {img_path} does not exist')
                continue
            if os.path.exists(args.output_folder) and  len(os.listdir(args.output_folder))!=0:
                continue
            ...
            for frame in tqdm(range(start, end+1)):
                image= os.path.join(img_path, f'{int(frame):06d}.jpg')
